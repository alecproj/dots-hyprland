...
- загружать модули и секции из явного registry;
...
Все директории модулей используют один контракт. Расположение файла не определяет секцию TUI: секция, порядок и точный путь каждого модуля задаются в `registry.py`.
...
5. `additions/registry.py` для секций, их переводов, порядка и путей модулей.
...
- локализация `en` и `ru`;
- цвета curses;
- секции, явно зарегистрированные в `registry.py`;
- сворачивание секций;
...
`additions/registry.py` является единственным источником структуры TUI. Каждая
секция задаётся явно:
...
SectionMeta(
    ...
    module_paths=(
        ...
        "modules/another-embedded-module.sh",
    ),
)
```

`module_paths` содержит точные пути относительно `additions/`. Каталог не имеет
семантического значения и не выбирает секцию автоматически. Неизвестные секции,
автоматическое преобразование slug в заголовок и сканирование каталогов не используются.

Для каждого зарегистрированного файла выполняется:
...
- хранит порядок секций и модулей;
- хранит английское и русское название каждой секции;
- требует явной регистрации точного пути модуля;
- валидирует типы JSON, включая массивы;
- требует совпадение `MODULE_ID` с именем файла;
- требует совпадение `MODULE_SECTION` с секцией registry;
